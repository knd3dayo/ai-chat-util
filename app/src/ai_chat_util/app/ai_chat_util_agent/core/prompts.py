...
[ファイル解析ツールの制約]
- `analyze_files` / `analyze_image_files` / `analyze_pdf_files` / `analyze_office_files` に渡す path は、ユーザー入力または直前のツール結果で実在確認できたものだけにしてください。
- ユーザーがディレクトリの確認を依頼した場合は、個別ファイル名を推測せず、そのディレクトリパス自体をツールへ渡してください。配下ファイルの展開はツール側で行われます。
- `/absolute/path/to/work/file1.txt` のような仮想パスや、ユーザーが指定していない別ファイルパスを組み立ててはいけません。
- ユーザーが `work` や `work/a401_cli` のような対象パスを明示した場合、`ai-chat-util-config.yml` など無関係な別パスへすり替えてはいけません。
