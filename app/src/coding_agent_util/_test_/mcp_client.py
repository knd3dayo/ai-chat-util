...
Server (streamable-http) example:
    ...
    uv run -m coding_agent_util.mcp.mcp_server --mode http --host 127.0.0.1 -p 7101

Client example:

    uv run -m coding_agent_util._test_.mcp_client \
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--wait", action="store_true", help="After execute, poll status until the task exits")
    p.add_argument(
        "--poll-interval",
        type=float,
        default=0.25,
    ...
    p.add_argument("--tail", type=int, default=50, help="Tail lines when polling status")
    p.add_argument(
        "--cancel-after",
        type=float,
        default=0.0,
...
async def _run() -> int:
    ...
    cancel_at = time.time() + float(args.cancel_after) if args.cancel_after and args.cancel_after > 0 else None
    final: Optional[dict[str, Any]] = None
    for i in range(int(args.max_polls)):
        st = await client.status(task_id, tail=args.tail)
        if i % 10 == 0:
            print(f"poll[{i}] status={st.get('status')} sub_status={st.get('sub_status')}")
