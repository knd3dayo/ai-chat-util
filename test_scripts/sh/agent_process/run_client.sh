...
uv --directory "$project_dir" run -m ai_chat_util.cli --config "$AI_CHAT_UTIL_CONFIG" --loglevel INFO --logfile chat_timeout_5s.log chat --use_mcp -p "自律側エージェントMCPツールで、/srv/ai_platform/workspaces/e2e_sv_ws_1/done.txtに「完了2」と書き込んで.最後にget_resultでログを出力して"
