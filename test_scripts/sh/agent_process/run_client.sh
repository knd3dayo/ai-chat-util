...
uv --directory "$project_dir" run -m ai_chat_util.cli --config "$AI_CHAT_UTIL_CONFIG" --loglevel INFO --logfile chat_timeout_5s.log agent_chat -p "コーディングエージェントMCPツールで、/srv/ai_platform/workspaces/e2e_sv_ws_1/done.txtに「完了2」と書き込んで.最後にget_resultでログを出力して"
